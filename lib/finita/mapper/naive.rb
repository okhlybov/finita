...
class Mapper::Naive < Mapper
  ...
  class Code < Mapper::Code
    ...
    def write_defs(stream)
      ...
      if @mapper.mpi?
        if @system_code.complex?
          c_type = Finita::CType[Float]
        ...
        $
        stream << %$
          void #{synchronizeArray}(#{@numericArray.type}* array) {
            int ierr, index, count, process;
            #{c_type} *input, *real;
            input = (#{c_type}*)#{malloc}(#{counts}[FinitaProcessIndex]*sizeof(#{c_type})); #{assert}(input);
            real = (#{c_type}*)#{malloc}(#{@nodeArray.size}(&#{nodes})*sizeof(#{c_type})); #{assert}(real);
        ...
        if @system_code.complex?
          stream << %${
            ...
            imaginary = (#{c_type}*)#{malloc}(#{@nodeArray.size}(&#{nodes})*sizeof(#{c_type})); #{assert}(imaginary);
            for(count = 0, index = #{offsets}[FinitaProcessIndex]; count < #{counts}[FinitaProcessIndex]; ++count, ++index) {
              input[count] = creal(#{@numericArray.get}(array, index));
            }
            ierr = MPI_Allgatherv(input, #{counts}[FinitaProcessIndex], #{mpi_type}, real, #{counts}, #{offsets}, #{mpi_type}, MPI_COMM_WORLD); #{assert}(ierr == MPI_SUCCESS);
            for(count = 0, index = #{offsets}[FinitaProcessIndex]; count < #{counts}[FinitaProcessIndex]; ++count, ++index) {
              input[count] = cimag(#{@numericArray.get}(array, index));
            }
            ierr = MPI_Allgatherv(input, #{counts}[FinitaProcessIndex], #{mpi_type}, imaginary, #{counts}, #{offsets}, #{mpi_type}, MPI_COMM_WORLD); #{assert}(ierr == MPI_SUCCESS);
            for(process = 0; process < FinitaProcessCount; ++process) {
              if(process != FinitaProcessIndex) {
                for(count = 0, index = #{offsets}[process]; count < #{counts}[process]; ++count, ++index) {
                  #{@numericArray.set}(array, index, real[index] + _Complex_I*imaginary[index]);
                }
              }
            }
            #{free}(imaginary);
          ...
          stream << %$
            for(count = 0, index = #{offsets}[FinitaProcessIndex]; count < #{counts}[FinitaProcessIndex]; ++count, ++index) {
              input[count] = #{@numericArray.get}(array, index);
            }
            ierr = MPI_Allgatherv(input, #{counts}[FinitaProcessIndex], #{mpi_type}, real, #{counts}, #{offsets}, #{mpi_type}, MPI_COMM_WORLD); #{assert}(ierr == MPI_SUCCESS);
            for(process = 0; process < FinitaProcessCount; ++process) {
              if(process != FinitaProcessIndex) {
                for(count = 0, index = #{offsets}[process]; count < #{counts}[process]; ++count, ++index) {
                  #{@numericArray.set}(array, index, real[index]);
                }
              }
            }
          $
        ...
        stream << %$
          void #{gatherArray}(#{@numericArray.type}* array) {
            int ierr, index, count, process;
            #{c_type} *input, *real;
            input = (#{c_type}*)#{malloc}(#{counts}[FinitaProcessIndex]*sizeof(#{c_type})); #{assert}(input);
            real = (#{c_type}*)#{malloc}(#{@nodeArray.size}(&#{nodes})*sizeof(#{c_type})); #{assert}(real);
        ...
        if @system_code.complex?
          stream << %${
            ...
            ierr = MPI_Gatherv(input, #{counts}[FinitaProcessIndex], #{mpi_type}, real, #{counts}, #{offsets}, #{mpi_type}, 0, MPI_COMM_WORLD); #{assert}(ierr == MPI_SUCCESS);
            FINITA_NHEAD for(count = 0, index = #{offsets}[FinitaProcessIndex]; count < #{counts}[FinitaProcessIndex]; ++count, ++index) {
              input[count] = cimag(#{@numericArray.get}(array, index));
            }
            ierr = MPI_Gatherv(input, #{counts}[FinitaProcessIndex], #{mpi_type}, imaginary, #{counts}, #{offsets}, #{mpi_type}, 0, MPI_COMM_WORLD); #{assert}(ierr == MPI_SUCCESS);
        ...
        stream << %$
          void #{scatterArray}(#{@numericArray.type}* array) {
            int ierr, count, index, process;
            #{c_type} *output, *real;
            size_t size = #{size}();
            output = (#{c_type}*)#{malloc}(#{@nodeArray.size}(&#{nodes})*sizeof(#{c_type})); #{assert}(output);
            real = (#{c_type}*)#{malloc}(#{@nodeArray.size}(&#{nodes})*sizeof(#{c_type})); #{assert}(real);
        $
        if @system_code.complex?
            stream << %${
            ...
            FINITA_HEAD for(index = 0; index < size; ++index) {
              output[index] = creal(#{@numericArray.get}(array, index));
            }
            ierr = MPI_Scatterv(output, #{counts}, #{offsets}, #{mpi_type}, real, #{counts}[FinitaProcessIndex], #{mpi_type}, 0, MPI_COMM_WORLD); #{assert}(ierr == MPI_SUCCESS);
            FINITA_HEAD for(count = 0, index = #{offsets}[FinitaProcessIndex]; count < #{counts}[FinitaProcessIndex]; ++count, ++index) {
              output[index] = cimag(#{@numericArray.get}(array, index));
            }
            ierr = MPI_Scatterv(output, #{counts}, #{offsets}, #{mpi_type}, imaginary, #{counts}[FinitaProcessIndex], #{mpi_type}, 0, MPI_COMM_WORLD); #{assert}(ierr == MPI_SUCCESS);
          ...
          stream << %$
            FINITA_HEAD for(index = 0; index < size; ++index) {
              output[index] = #{@numericArray.get}(array, index);
            }
            ierr = MPI_Scatterv(output, #{counts}, #{offsets}, #{mpi_type}, real, #{counts}[FinitaProcessIndex], #{mpi_type}, 0, MPI_COMM_WORLD); #{assert}(ierr == MPI_SUCCESS);
            FINITA_NHEAD for(process = 1; process < FinitaProcessCount; ++process) {
              for(count = 0, index = #{offsets}[process]; count < #{counts}[process]; ++count, ++index) {
